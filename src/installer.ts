...
const WIN_X64_URL =
  'https://raw.githubusercontent.com/daipeihust/im-select/master/win/out/x64/im-select.exe';
const WIN_X86_URL =
  'https://raw.githubusercontent.com/daipeihust/im-select/master/win/out/x86/im-select.exe';

export async function checkAndInstallImSelect(
...
async function handleMacInstallation(
      ...
      return await vscode.window.withProgress(
        ...
        async (progress) => {
          try {
            progress.report({
              increment: 0,
              message: 'Downloading Homebrew installer...',
            });
            await execCommand(
            ...
            progress.report({
              increment: 100,
              message: 'Homebrew installation complete.',
            });
            return true;
          } catch (error) {
            const retry = await vscode.window.showErrorMessage(
              'Failed to install Homebrew: ' +
                error +
                '. You may retry or install Homebrew manually.',
              'Retry'
            );
            if (retry === 'Retry') {
              return await handleMacInstallation(config, reinstall);
    ...
    return await vscode.window.withProgress(
      ...
      async (progress) => {
        try {
          progress.report({
            increment: 0,
            message: 'Tapping Homebrew repository...',
          });
          await execCommand('brew tap daipeihust/tap');
          progress.report({
            increment: 50,
            message: 'Installing im-select...',
          });
          ...
          const retry = await vscode.window.showErrorMessage(
            'Failed to install im-select: ' + error,
            'Retry'
          );
          if (retry === 'Retry') {
            return await handleMacInstallation(config, reinstall);
...
async function handleWindowsInstallation(
  config: vscode.WorkspaceConfiguration,
  reinstall: boolean
): Promise<boolean> {
  const appDataPath =
