...
async function toggleExtension() {
    const config = vscode.workspace.getConfiguration(
        'i-wanna-english.autoSwitchInputMethod'
    );
    ...
    if (!newEnabled) {
        const disableIM = config.get<string>('disableIM');
        if (disableIM) {
            try {
                ...
                console.error(
                    'Failed to switch to disable input method:',
                    error
                );
            }
        }
    ...
    await config.update('enable', newEnabled, true);
...
async function manuallyInstallImSelect() {
    ...
    if (installed) {
        vscode.window.showInformationMessage(
            'im-select installed successfully.'
        );
    } else {
...
async function autoDetectAndConfigure() {
    ...
    if (detectedIM) {
        ...
        vscode.window.showErrorMessage(
            'Failed to detect a suitable input method.'
        );
    }
}
